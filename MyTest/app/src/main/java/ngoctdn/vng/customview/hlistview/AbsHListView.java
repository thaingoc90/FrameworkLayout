package ngoctdn.vng.customview.hlistview;
...
import ngoctdn.vng.customview.hlistview.util.ViewHelperFactory.ViewHelper;
import ngoctdn.vng.customview.hlistview.util.v11.MultiChoiceModeListener;
import ngoctdn.vng.customview.hlistview.util.v11.MultiChoiceModeWrapper;
...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener,
     ...
     */
    int mSelectorPosition = AdapterView.INVALID_POSITION;
     ...
     */
    protected int mResurrectToPosition = AdapterView.INVALID_POSITION;
    ...
    public AbsHListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (AdapterView.LOG_ENABLED) {
            Log.i(AdapterView.LOG_TAG, "defStyle: " + defStyle);
        }
     ...
     * list, or <code>null</code> if the choice mode is set to {#CHOICE_MODE_NONE}.
     */
     ...
     * {#CHOICE_MODE_NONE} and the adapter has stable IDs. ({@link ListAdapter#hasStableIds()} == {@code true})
     *
    ...
    /**
     * Defines the choice behavior for the List. By default, Lists do not have any choice behavior ({#CHOICE_MODE_NONE}). By
     * setting the choiceMode to {#CHOICE_MODE_SINGLE}, the List allows up to one item to be in a chosen state. By setting the
     * choiceMode to {#CHOICE_MODE_MULTIPLE}, the list allows any number of items to be chosen.
     *
     * @param choiceMode One of {#CHOICE_MODE_NONE}, {#CHOICE_MODE_SINGLE}, or {#CHOICE_MODE_MULTIPLE}
     */
    @TargetApi(11)
     ...
     * choice mode is set to {#CHOICE_MODE_MULTIPLE_MODAL}.
     *
    ...
    public Parcelable onSaveInstanceState() {
        ...
        if (selectedId >= 0) {
            ...
            ss.firstId = AdapterView.INVALID_POSITION;
        } else {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (ss.selectedId >= 0) {
            ...
            mSelectorPosition = AdapterView.INVALID_POSITION;
            mNeedSync = true;
            mPendingSync = ss;
    ...
    protected void resetList() {
        ...
        mOldSelectedColId = AdapterView.INVALID_COL_ID;
        setSelectedPositionInt(AdapterView.INVALID_POSITION);
        setNextSelectedPositionInt(AdapterView.INVALID_POSITION);
        mSelectedLeft = 0;
        mSelectorPosition = AdapterView.INVALID_POSITION;
            ...
            switch (action) {
                case AccessibilityNodeInfoCompat.ACTION_CLEAR_SELECTION: {
                    if (getSelectedItemPosition() == position) {
                        setSelection(AdapterView.INVALID_POSITION);
                        return true;
                    }
                }
                return false;
    ...
    private class PerformClick extends WindowRunnnable implements Runnable {
        ...
        public void run() {
            ...
            if (adapter != null && mItemCount > 0 &&
                    motionPosition != AdapterView.INVALID_POSITION &&
    ...
    public long pointToColId(int x, int y) {
        ...
        return AdapterView.INVALID_COL_ID;
    }
        ...
        if (mTouchMode == TOUCH_MODE_SCROLL) {
            ...
            if (x != mLastX) {
                ...
                if (overScrollDistance != 0) {
                    ...
                    final int overscrollMode = getOverScrollMode();
                    if (overscrollMode == View.OVER_SCROLL_ALWAYS || (overscrollMode == View.OVER_SCROLL_IF_CONTENT_SCROLLS && !contentFits())) {
                        if (rawDeltaX > 0) {
    ...
    private class FlingRunnable implements Runnable {
         ...
         */
        private final ngoctdn.vng.customview.hlistview.OverScroller mScroller;
    ...
    public class PositionScroller implements Runnable {
        ...
        void start(final int position) {
            ...
            if (clampedPosition < firstPos) {
                ...
                scrollToVisible(clampedPosition, AdapterView.INVALID_POSITION, SCROLL_DURATION);
                return;
            }
            ...
            mBoundPos = AdapterView.INVALID_POSITION;
            mLastSeenPos = AdapterView.INVALID_POSITION;
        ...
        void start(final int position, final int boundPosition) {
            ...
            if (boundPosition == AdapterView.INVALID_POSITION) {
                start(position);
                return;
            }
        ...
        void scrollToVisible(int targetPos, int boundPos, int duration) {
            ...
            if (boundPos < firstPos || boundPos > lastPos) {
                ...
                boundPos = AdapterView.INVALID_POSITION;
            }
                ...
                mClearScrollingCache = new Runnable() {
                    ...
                    public void run() {
                        if (mCachingStarted) {
                            ...
                            if ((getPersistentDrawingCache() & ViewGroup.PERSISTENT_SCROLLING_CACHE) == 0) {
                                setChildrenDrawingCacheEnabled(false);
                            }
    ...
    boolean trackMotionScroll(int deltaX, int incrementalDeltaX) {
        ...
        if (!inTouchMode && mSelectedPosition != AdapterView.INVALID_POSITION) {
            final int childIndex = mSelectedPosition - mFirstPosition;
            if (childIndex >= 0 && childIndex < getChildCount()) {
                positionSelector(mSelectedPosition, getChildAt(childIndex));
            }
        } else if (mSelectorPosition != AdapterView.INVALID_POSITION) {
            final int childIndex = mSelectorPosition - mFirstPosition;
            if (childIndex >= 0 && childIndex < getChildCount()) {
                positionSelector(AdapterView.INVALID_POSITION, getChildAt(childIndex));
            }
        } else {
            mSelectorRect.setEmpty();
        }
    ...
    protected int findClosestMotionCol(int x) {
        ...
        return motionCol != AdapterView.INVALID_POSITION ? motionCol : mFirstPosition + childCount - 1;
    }
    ...
    boolean resurrectSelection() {
        ...
        mResurrectToPosition = AdapterView.INVALID_POSITION;
        removeCallbacks(mFlingRunnable);
        if (mPositionScroller != null) {
            mPositionScroller.stop();
        }
    ...
    protected void handleDataChanged() {
        ...
        if (count > 0) {
            ...
            if (mNeedSync) {
                ...
                switch (mSyncMode) {
                    case AdapterView.SYNC_SELECTED_POSITION:
                        if (isInTouchMode()) {
                        ...
                        break;
                    case AdapterView.SYNC_FIRST_POSITION:
        ...
        mSelectedColId = AdapterView.INVALID_COL_ID;
        mNextSelectedPosition = AdapterView.INVALID_POSITION;
        mNextSelectedColId = AdapterView.INVALID_COL_ID;
        mNeedSync = false;
        mPendingSync = null;
        mSelectorPosition = AdapterView.INVALID_POSITION;
    ...
    public class RecycleBin {
        ...
        public void addScrapView(View scrap, int position) {
            ...
            if (!shouldRecycleViewType(viewType) || scrapHasTransientState) {
                if (viewType != AdapterView.ITEM_VIEW_TYPE_HEADER_OR_FOOTER || scrapHasTransientState) {
